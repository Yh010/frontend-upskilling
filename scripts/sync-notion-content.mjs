...
const videoOutputFile = new URL("../src/content/notionVideos.generated.ts", import.meta.url);
...
const videoDatabaseId = process.env.NOTION_VIDEO_DATABASE_ID;
...
async function queryDatabasePages(id) {
  const database = await request(`/databases/${id}`);
  const sourceId = database.data_sources?.[0]?.id;
  if (!sourceId) throw new Error("The Notion video database does not contain a data source.");
  ...
  do {
    ...
    const response = await request(`/data_sources/${sourceId}/query`, { method: "POST", body: JSON.stringify(body) });
    pages.push(...response.results);
    pageCursor = response.has_more ? response.next_cursor : undefined;
  } while (pageCursor);
...
const notionVideos = videoDatabaseId ? await Promise.all((await queryDatabasePages(videoDatabaseId))
  .filter((page) => page.properties.Published?.checkbox !== false)
  .map(async (page) => {
    const properties = page.properties;
    const href = properties["YouTube URL"]?.url;
    const playlistTitle = properties.Playlist?.select?.name || propertyText(properties.Playlist);
    if (!href || !playlistTitle) return undefined;

    const metadata = await videoMetadata(href);
    const videoId = youTubeVideoId(href) || page.id.replace(/-/g, "");
    return {
      id: page.id,
      title: propertyText(properties.Name) || metadata.title || "YouTube video",
...
const videoSource = `import type { NotionYouTubeVideo } from "./writing";\n\n// Generated by scripts/sync-notion-content.mjs. Do not edit manually.\nexport const notionYouTubeVideos: NotionYouTubeVideo[] = ${JSON.stringify(notionVideos.filter(Boolean), null, 2)};\n`;
await writeFile(videoOutputFile, videoSource);
